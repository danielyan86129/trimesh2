...
template <class T>
extern void diffuse_vector(TriMesh* themesh, ::std::vector<T>& field,
                           float sigma);
...
extern void diagonalize_curv(const vec& old_u, const vec& old_v, float ku,
                             float kuv, float kv, const vec& new_norm,
...
// new_u, new_v basis.
extern void proj_curv(const vec& old_u, const vec& old_v, float old_ku,
                      float old_kuv, float old_kv, const vec& new_u,
                      const vec& new_v, float& new_ku, float& new_kuv,
                      float& new_kv);

// Like the above, but for dcurv
extern void proj_dcurv(const vec& old_u, const vec& old_v,
                       const Vec<4> old_dcurv, const vec& new_u,
                       const vec& new_v, Vec<4>& new_dcurv);
...
extern void
select_big_comps(TriMesh* mesh, const ::std::vector<int>& comps,
...
extern void
select_small_comps(TriMesh* mesh, const ::std::vector<int>& comps,
...
extern void find_overlap(TriMesh* mesh1, TriMesh* mesh2, float& area,
                         float& rmsdist);

extern void find_overlap(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1,
                         const xform& xf2, float& area, float& rmsdist);
...
extern float iou(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1,
                 const xform& xf2);
...
extern TriMesh* make_helix(int tess_th, int tess_ph, float turns,
                           float r = 0.2f);
...
// around the z axis.
extern TriMesh*
make_surface_of_revolution(int tess_th, const ::std::vector<point>& curve_pts);
...
extern TriMesh* make_teapot(int tess, bool omit_bottom = false,
                            bool taller = false);
