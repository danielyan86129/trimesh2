...
private:
    ...
    void mouse_click(int mousex, int mousey, const point& scene_center,
                     float scene_size);

public:
    GLCamera()
        : lastb(Mouse::NONE), lightdir(vec(0, 0, 1)), dospin(false),
          spinspeed(0), constraint_(UNCONSTRAINED), field_of_view(0.7f),
