...
public:
    ...
    typedef typename ::std::conditional<::std::is_integral<T>::value, double,
                                        T>::type float_type;
...
public:
    ...
    inline explicit Vec(
        S x_,
        typename ::std::enable_if<::std::is_arithmetic<S>::value, void>::type* =
            0)
    {
    ...
    template <class S>
    inline explicit Vec(
        const S& v_, typename ::std::enable_if<!::std::is_arithmetic<S>::value,
                                               void>::type* = 0)
    {
        for (size_type i = 0; i < D; i++)
            v[i] = v_[i];
    ...
    template <class S>
    inline
        typename ::std::enable_if<!::std::is_arithmetic<S>::value, Vec&>::type
        operator=(const S& v_)
    {
        for (size_type i = 0; i < D; i++)
            v[i] = v_[i];
...
// Cross product
template <class T>
static inline const Vec<3, T> operator%(const Vec<3, T>& v1,
                                        const Vec<3, T>& v2)
{
...
// Distance
template <size_t D, class T>
static inline const typename Vec<D, T>::float_type dist(const Vec<D, T>& v1,
                                                        const Vec<D, T>& v2)
{
    using namespace ::std;
...
template <size_t D, class T>
static inline const typename Vec<D, T>::float_type distance(const Vec<D, T>& v1,
                                                            const Vec<D, T>& v2)
{
    return dist(v1, v2);
}
...
template <class T>
static inline Vec<3, T> trinorm(const Vec<3, T>& v0, const Vec<3, T>& v1,
                                const Vec<3, T>& v2)
{
...
template <size_t D, class T>
static inline const typename Vec<D, T>::float_type angle(const Vec<D, T>& v1,
                                                         const Vec<D, T>& v2)
{
    using namespace ::std;
...
#define VEC_DECLARE_TWOARG_SV(name)                                            \
    template <::std::size_t D, class T>                                        \
    static inline trimesh::Vec<D, T> name(const T& a,                          \
                                          const trimesh::Vec<D, T>& v)         \
    {                                                                          \
        using namespace ::std;                                                 \
        using namespace ::trimesh;                                             \
        Vec<D, T> result(VEC_UNINITIALIZED);                                   \
...
#define VEC_DECLARE_THREEARG_SSV(name)                                         \
    template <::std::size_t D, class T>                                        \
    static inline trimesh::Vec<D, T> name(const T& a, const T& b,              \
                                          const trimesh::Vec<D, T>& v)         \
    {                                                                          \
        using namespace ::std;                                                 \
        using namespace ::trimesh;                                             \
...
template <size_t D, class T>
static inline void swap(const ::trimesh::Vec<D, T>& v1,
                        const ::trimesh::Vec<D, T>& v2)
{
    for (size_t i = 0; i < D; i++)
        swap(v1[i], v2[i]);
}
