...
template <class T, int N>
static inline void lubksb(const T (&A)[N][N], const int ind[N], const T b[N],
                          T x[N])
{
    if (&x[0] != &b[0])
    {
...
template <class T, int N>
static inline void ldltsl(const T (&A)[N][N], const T rdiag[N], const T b[N],
                          T x[N])
{
    for (int i = 0; i < N; i++)
    {
...
template <class T, int N>
static inline void eigmult(const T (&A)[N][N], const T d[N], const T b[N],
                           T x[N])
{
    T e[N];
    for (int i = 0; i < N; i++)
...
static inline void svd(T (&A)[M][N], T s[N], T (&V)[N][N])
{
    ...
    while (p > 0)
    {
        ...
        switch (kase)
        {
            ...
            case 3:
            {
                T scale = 1 / max(max(max(max(abs(s[p - 1]), abs(s[p - 2])),
                                          abs(e[p - 2])),
                                      abs(s[k])),
                                  abs(e[k]));
                T sp = s[p - 1] * scale;
