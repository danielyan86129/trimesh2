...
public:
    ...
    template <class T>
    KDtree(const ::std::vector<T>& v) : root(NULL), storage(NULL)
    {
    ...
    template <class T>
    KDtree(::std::vector<T*>& pts) : root(NULL), storage(NULL)
    {
    ...
    const float* closest_to_pt(const float* p, float maxdist2 = 0.0f,
                               const CompatFunc* iscompat = NULL,
                               float approx_eps = 0.0f) const;

    // Shorthand with approx_eps but no iscompat
    const float* closest_to_pt(const float* p, float maxdist2,
