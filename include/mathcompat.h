...
#if _MSC_VER < 1700
static inline float acoshf(float x) { return log(x + sqrt(x * x - 1)); }
static inline double acosh(double x) { return log(x + sqrt(x * x - 1)); }
static inline long double acoshl(long double x)
{
    return log(x + sqrt(x * x - 1));
}

static inline float asinhf(float x) { return log(x + sqrt(x * x + 1)); }
static inline double asinh(double x) { return log(x + sqrt(x * x + 1)); }
static inline long double asinhl(long double x)
{
    return log(x + sqrt(x * x + 1));
}

static inline float atanhf(float x) { return (log(1 + x) - log(1 - x)) / 2; }
static inline double atanh(double x) { return (log(1 + x) - log(1 - x)) / 2; }
...
static inline float truncf(float x) { return (x < 0) ? ceil(x) : floor(x); }
static inline double trunc(double x) { return (x < 0) ? ceil(x) : floor(x); }
...
#if _MSC_VER < 1800
static inline float exp2f(float x) { return exp(0.6931472f * x); }
static inline double exp2(double x) { return exp(0.69314718055994530942 * x); }
...
static inline float log2f(float x) { return log(1.4426950f * x); }
static inline double log2(double x) { return log(1.4426950408889634074 * x); }
static inline long double log2l(long double x)
{
    return log(1.442695040888963407359924681001892137L * x);
}

static inline float fdimf(float x, float y) { return x > y ? x - y : 0.0f; }
...
#include <type_traits>
#elif !defined(_LIBCPP_TYPE_TRAITS) && !defined(_TR1_TYPE_TRAITS) &&           \
    !defined(_GLIBCXX_TYPE_TRAITS)
namespace std
{
...
template <class T>
struct is_integral
    : integral_constant<bool, ::std::numeric_limits<T>::is_integer>
{
};
template <class T, int N>
struct is_integral<T[N]> : false_type
{
};
template <class T>
...
struct is_floating_point
    : integral_constant<bool, ::std::numeric_limits<T>::is_specialized &&
                                  !::std::numeric_limits<T>::is_integer>
{
};
template <class T, int N>
struct is_floating_point<T[N]> : false_type
{
};
template <class T>
