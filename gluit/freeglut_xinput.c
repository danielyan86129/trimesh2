...
void fgPrintXILeaveEvent(XILeaveEvent* event)
{
    ...
    printf("    modifiers: locked 0x%x latched 0x%x base 0x%x\n",
           event->mods.locked, event->mods.latched, event->mods.base);
    printf("    group: locked 0x%x latched 0x%x base 0x%x\n",
           event->group.locked, event->group.latched, event->group.base);
...
{
    XEvent std_ev; /* standard single-pointer event to be added to the event
                      queue */
    int i, button = 0;
    ...
    if (XGetEventData(fgDisplay.Display, cookie) &&
        (cookie->type == GenericEvent) && (cookie->extension == xi_opcode))
    {
        ...
        switch (cookie->evtype)
        {
            ...
            case XI_Leave:
                ...
                std_ev.type =
                    evcross->evtype == XI_Enter ? EnterNotify : LeaveNotify;
                ...
                std_ev.xcrossing.state =
                    BUTTON_MASK(*(unsigned int*)evcross->buttons.mask);
            ...
            case XI_ButtonRelease:
                ...
                INVOKE_WCB(
                    *window, MultiButton,
                    (event->deviceid, event->event_x, event->event_y,
                     event->detail - 1,
                     (event->evtype == XI_ButtonPress ? GLUT_DOWN : GLUT_UP)));

                /* Also process the standard button event */
                std_ev.type = event->evtype == XI_ButtonPress ? ButtonPress
                                                              : ButtonRelease;
            ...
            case XI_Motion:
                ...
                std_ev.xmotion.state =
                    BUTTON_MASK(*(unsigned int*)event->buttons.mask);
