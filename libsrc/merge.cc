...
TriMesh *join(const vector<TriMesh *> &meshes, float tol) {
  ...
  for (int i = 0; i < nmeshes; i++) {
    ...
    outmesh->vertices.insert(outmesh->vertices.end(), m->vertices.begin(),
                             m->vertices.end());

    if (have_colors && !m->colors.empty()) {
      outmesh->colors.insert(outmesh->colors.end(), m->colors.begin(),
                             m->colors.end());
    ...
    if (have_confidences && !m->confidences.empty()) {
      outmesh->confidences.insert(outmesh->confidences.end(),
                                  m->confidences.begin(), m->confidences.end());
    ...
    if (have_normals) {
      if (m->normals.empty())
        m->need_normals();
      outmesh->normals.insert(outmesh->normals.end(), m->normals.begin(),
                              m->normals.end());
    }

    // Append and renumber faces
    int onf = outmesh->faces.size();
    outmesh->faces.insert(outmesh->faces.end(), m->faces.begin(),
                          m->faces.end());
