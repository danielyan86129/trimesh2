...
static inline GLint myGluUnProject(GLdouble winX, GLdouble winY, GLdouble winZ,
  ...
  dvec3 v =
      xf * dvec3((winX - view[0]) / view[2] * 2.0 - 1.0,
                 (winY - view[1]) / view[3] * 2.0 - 1.0, winZ * 2.0 - 1.0);
  *objX = v[0];
  *objY = v[1];
  *objZ = v[2];
  return GL_TRUE;
...
bool GLCamera::read_depth(int x, int y, point &p) const {
  ...
  static const float dx[] = {0, 1,  -1, -1, 1,  3,   -3, 0, 0,
                             6, -6, -6, 6,  25, -25, 0,  0};
  static const float dy[] = {0, 1, 1,  -1, -1, 0, 0,  3,  -3,
                             6, 6, -6, -6, 0,  0, 25, -25};
...
void GLCamera::mouse_click(int mousex, int mousey, const point &scene_center,
                           float scene_size) {
  GLdouble M[16], P[16];
  GLint V[4];
#if 0
  ...
  myGluProject(scene_center[0], scene_center[1], scene_center[2], M, P, V, &cx,
               &cy, &cz);
  ...
  GLdouble sx, sy, sz;
  myGluUnProject((xmin + xmax) / 2, (ymin + ymax) / 2, 1, M, P, V, &sx, &sy,
                 &sz);
  ...
  myGluProject(spincenter[0], spincenter[1], spincenter[2], M, P, V, &cx, &cy,
               &cz);
...
void GLCamera::mouse(int mousex, int mousey, Mouse::button b,
                     const point &scene_center, float scene_size, xform &xf) {
