...
Can write: PLY (triangle mesh, range grid), OFF, OBJ, RAY, SM, STL, PTS, C++,
DAE
*/

#include "trimesh/TriMesh.h"
#include "trimesh/endianutil.h"
...
#define COND_READ(cond, where, len)                                            \
  do {                                                                         \
    if ((cond) && !fread((void *)&(where), (len), 1, f))                       \
      return false;                                                            \
  } while (0)
#define FPRINTF(...)                                                           \
  do {                                                                         \
...
static bool read_verts_bin(FILE *f, TriMesh *mesh, bool &need_swap, int nverts,
                           int vert_len, int vert_pos, int vert_norm,
                           int vert_color, bool float_color, int vert_conf);
static bool slurp_verts_bin(FILE *f, TriMesh *mesh, bool need_swap, int nverts);
static bool read_verts_asc(FILE *f, TriMesh *mesh, int nverts, int vert_len,
                           int vert_pos, int vert_norm, int vert_color,
                           bool float_color, int vert_conf);
static bool read_faces_bin(FILE *f, TriMesh *mesh, bool need_swap, int nfaces,
                           int face_len, int face_count, int face_idx);
static bool read_faces_asc(FILE *f, TriMesh *mesh, int nfaces, int face_len,
                           int face_count, int face_idx,
...
static bool write_ply_binary(TriMesh *mesh, FILE *f, bool little_endian,
                             bool write_norm, bool write_grid,
                             bool float_color);
...
static bool write_verts_asc(TriMesh *mesh, FILE *f, const char *before_vert,
                            const char *before_norm, const char *before_color,
                            bool float_color, const char *before_conf,
...
static bool write_verts_bin(TriMesh *mesh, FILE *f, bool need_swap,
                            ...
                            bool write_conf);
static bool write_faces_asc(TriMesh *mesh, FILE *f, const char *before_face,
                            const char *after_line);
...
static bool read_ply(FILE *f, TriMesh *mesh) {
  ...
  while (LINE_IS("property")) {
    if (LINE_IS("property float x") || LINE_IS("property float32 x"))
      vert_pos = vert_len;
    if (LINE_IS("property float nx") || LINE_IS("property float32 nx"))
      vert_norm = vert_len;
  ...
  if (LINE_IS("element face")) {
    ...
    while (LINE_IS("property")) {
      ...
      if (sscanf(buf, "property list %255s %255s vertex_ind", count_type,
                 ind_type) == 2) {
  ...
  while (!LINE_IS("end_header"))
    GET_LINE();
  if (binary && buf[10] == '\r') {
    eprintf("Warning: possibly corrupt file. (Transferred as ASCII instead of "
            "BINARY?)\n");
  }
  ...
  if (binary) {
    if (!read_verts_bin(f, mesh, need_swap, nverts, vert_len, vert_pos,
                        vert_norm, vert_color, float_color, vert_conf))
      return false;
  } else {
    if (!read_verts_asc(f, mesh, nverts, vert_len, vert_pos, vert_norm,
                        vert_color, float_color, vert_conf))
      return false;
  }
  ...
  if (ngrid) {
    ...
    if (binary) {
      if (!read_faces_bin(f, mesh, need_swap, nfaces, face_len, face_count,
                          face_idx))
        return false;
    } else {
      if (!read_faces_asc(f, mesh, nfaces, face_len, face_count, face_idx))
        return false;
    }
  }
...
static bool read_3ds(FILE *f, TriMesh *mesh) {
  ...
  while (1) {
    ...
    unsigned chunklen;
    if (!fread(&chunkid, 2, 1, f) || !fread(&chunklen, 4, 1, f))
      return (feof(f) != 0);
    if (need_swap) {
      swap_ushort(chunkid);
...
static bool read_obj(FILE *f, TriMesh *mesh) {
  ...
  while (1) {
    ...
    } else if (LINE_IS("f ") || LINE_IS("f\t") || LINE_IS("t ") ||
               LINE_IS("t\t")) {
      thisface.clear();
      char *c = buf;
      while (1) {
...
static int ply_type_len(const char *buf, bool binary) {
  if (begins_with(buf, "char") || begins_with(buf, "uchar") ||
      begins_with(buf, "int8") || begins_with(buf, "uint8")) {
    return 1;
  } else if (begins_with(buf, "short") || begins_with(buf, "ushort") ||
             begins_with(buf, "int16") || begins_with(buf, "uint16")) {
    return (binary ? 2 : 1);
  } else if (begins_with(buf, "int") || begins_with(buf, "uint") ||
             begins_with(buf, "float") || begins_with(buf, "int32") ||
             begins_with(buf, "uint32") || begins_with(buf, "float32")) {
    return (binary ? 4 : 1);
...
static void check_need_swap(const point &p, bool &need_swap) {
  ...
  bool makes_sense = (p0 > -BIGNUM && p0 < BIGNUM && p1 > -BIGNUM &&
                      p1 < BIGNUM && p2 > -BIGNUM && p2 < BIGNUM);
  if (makes_sense)
    return;
  ...
  bool makes_sense_swapped = (p0 > -BIGNUM && p0 < BIGNUM && p1 > -BIGNUM &&
                              p1 < BIGNUM && p2 > -BIGNUM && p2 < BIGNUM);
...
static void tess(const vector<point> &verts, const vector<int> &thisface,
  ...
  if (thisface.size() == 4) {
    ...
    int i = (d02 < d13) ? 0 : 1;
    tris.push_back(TriMesh::Face(thisface[i], thisface[(i + 1) % 4],
                                 thisface[(i + 2) % 4]));
    tris.push_back(TriMesh::Face(thisface[i], thisface[(i + 2) % 4],
                                 thisface[(i + 3) % 4]));
    return;
  }
...
static bool write_ply_ascii(TriMesh *mesh, FILE *f, bool write_norm,
  ...
  if (!write_ply_header(mesh, f, "ascii", write_grid, write_tstrips, write_norm,
                        float_color))
    return false;
  if (!write_verts_asc(mesh, f, "", write_norm ? " " : 0, " ", float_color, " ",
                       ""))
    return false;
  if (write_grid) {
...
static bool write_ply_binary(TriMesh *mesh, FILE *f, bool little_endian,
  ...
  const char *format =
      little_endian ? "binary_little_endian" : "binary_big_endian";
  if (!write_ply_header(mesh, f, format, write_grid, write_tstrips, write_norm,
                        float_color))
    return false;
  if (!write_verts_bin(mesh, f, need_swap, write_norm, true, float_color, true))
    return false;
  if (write_grid) {
  ...
  if (!write_norm) {
    ...
    for (size_t i = 0; i < mesh->faces.size(); i++) {
      int n = fprintf(f, "f %d//%d %d//%d %d//%d\n", mesh->faces[i][0],
                      mesh->faces[i][0], mesh->faces[i][1], mesh->faces[i][1],
...
static bool write_pts(TriMesh *mesh, FILE *f) {
  ...
  for (size_t i = 0; i < mesh->vertices.size(); i++) {
    fprintf(f, "%f %f %f %f %f %f\n", mesh->vertices[i][0],
            mesh->vertices[i][1], mesh->vertices[i][2], mesh->normals[i][0],
...
static bool write_cc(TriMesh *mesh, FILE *f, const char *filename,
  ...
  FPRINTF(f,
          "\t::std::memcpy(&m->vertices[0][0], vertdata, sizeof(vertdata));\n");
  if (!mesh->colors.empty()) {
    FPRINTF(f, "\tm->colors.resize(%d);\n", nv);
    FPRINTF(
        f,
        "\t::std::memcpy(&m->colors[0][0], colordata, sizeof(colordata));\n");
  }
  if (write_norm) {
    FPRINTF(f, "\tm->normals.resize(%d);\n", nv);
    FPRINTF(
        f, "\t::std::memcpy(&m->normals[0][0], normdata, sizeof(normdata));\n");
  }
...
static bool write_dae(TriMesh *mesh, FILE *f) {
  ...
  FPRINTF(f, "<COLLADA xmlns=\"http://www.collada.org/2005/11/COLLADASchema\" "
             "version=\"1.4.1\">\n");
  FPRINTF(f, "<asset>\n");
...
static bool write_verts_asc(TriMesh *mesh, FILE *f, const char *before_vert,
                            ...
                            const char *after_line) {
  for (size_t i = 0; i < mesh->vertices.size(); i++) {
    FPRINTF(f, "%s%.7g %.7g %.7g", before_vert, mesh->vertices[i][0],
            mesh->vertices[i][1], mesh->vertices[i][2]);
    if (!mesh->normals.empty() && before_norm)
      FPRINTF(f, "%s%.7g %.7g %.7g", before_norm, mesh->normals[i][0],
              mesh->normals[i][1], mesh->normals[i][2]);
    if (!mesh->colors.empty() && before_color && float_color)
      FPRINTF(f, "%s%.7g %.7g %.7g", before_color, mesh->colors[i][0],
              mesh->colors[i][1], mesh->colors[i][2]);
    if (!mesh->colors.empty() && before_color && !float_color)
...
static bool write_verts_bin_helper(TriMesh *mesh, FILE *f, bool write_norm,
                                   bool write_color, bool float_color,
                                   bool write_conf) {
  if ((mesh->normals.empty() || !write_norm) &&
    ...
    for (size_t i = 0; i < mesh->vertices.size(); i++) {
      ...
      if (!mesh->colors.empty() && write_color && !float_color) {
        unsigned char c[3] = {color2uchar(mesh->colors[i][0]),
...
void TriMesh::set_verbose(int verbose_) { verbose = verbose_; }
