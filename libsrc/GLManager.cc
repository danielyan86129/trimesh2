...
typedef void(APIENTRYP glBufferData_t)(GLenum target, GLsizeiptr size,
...
typedef void(APIENTRYP glShaderSource_t)(GLuint shader, GLsizei count,
...
typedef void(APIENTRYP glGetShaderiv_t)(GLuint shader, GLenum pname,
                                        GLint *params);
static glGetShaderiv_t glGetShaderiv;
typedef void(APIENTRYP glGetShaderInfoLog_t)(GLuint shader, GLsizei bufSize,
...
typedef void(APIENTRYP glGetProgramiv_t)(GLuint program, GLenum pname,
                                         GLint *params);
static glGetProgramiv_t glGetProgramiv;
typedef void(APIENTRYP glGetProgramInfoLog_t)(GLuint program, GLsizei bufSize,
                                              GLsizei *length, GLchar *infoLog);
...
typedef void(APIENTRYP glUniform3f_t)(GLint location, GLfloat v0, GLfloat v1,
                                      GLfloat v2);
static glUniform3f_t glUniform3f;
typedef void(APIENTRYP glUniform4f_t)(GLint location, GLfloat v0, GLfloat v1,
...
typedef void(APIENTRYP glUniform3i_t)(GLint location, GLint v0, GLint v1,
                                      GLint v2);
static glUniform3i_t glUniform3i;
typedef void(APIENTRYP glUniform4i_t)(GLint location, GLint v0, GLint v1,
                                      GLint v2, GLint v3);
static glUniform4i_t glUniform4i;
typedef void(APIENTRYP glUniformMatrix2fv_t)(GLint location, GLsizei count,
                                             GLboolean transpose,
                                             const GLfloat *value);
static glUniformMatrix2fv_t glUniformMatrix2fv;
typedef void(APIENTRYP glUniformMatrix3fv_t)(GLint location, GLsizei count,
                                             GLboolean transpose,
                                             const GLfloat *value);
static glUniformMatrix3fv_t glUniformMatrix3fv;
typedef void(APIENTRYP glUniformMatrix4fv_t)(GLint location, GLsizei count,
                                             GLboolean transpose,
                                             const GLfloat *value);
static glUniformMatrix4fv_t glUniformMatrix4fv;
typedef void(APIENTRYP glGetActiveUniform_t)(GLuint program, GLuint index,
                                             GLsizei bufSize, GLsizei *length,
                                             GLint *size, GLenum *type,
                                             GLchar *name);
static glGetActiveUniform_t glGetActiveUniform;
typedef GLint(APIENTRYP glGetUniformLocation_t)(GLuint program,
                                                const GLchar *name);
...
typedef void(APIENTRYP glVertexAttrib3f_t)(GLuint index, GLfloat x, GLfloat y,
                                           GLfloat z);
static glVertexAttrib3f_t glVertexAttrib3f;
typedef void(APIENTRYP glVertexAttrib4f_t)(GLuint index, GLfloat x, GLfloat y,
                                           GLfloat z, GLfloat w);
static glVertexAttrib4f_t glVertexAttrib4f;
typedef void(APIENTRYP glVertexAttribPointer_t)(GLuint index, GLint size,
                                                GLenum type,
...
typedef void(APIENTRYP glGetActiveAttrib_t)(GLuint program, GLuint index,
                                            GLsizei bufSize, GLsizei *length,
                                            GLint *size, GLenum *type,
                                            GLchar *name);
static glGetActiveAttrib_t glGetActiveAttrib;
typedef GLint(APIENTRYP glGetAttribLocation_t)(GLuint program,
                                               const GLchar *name);
...
typedef void(APIENTRYP glFramebufferTexture2D_t)(GLenum target,
                                                 GLenum attachment,
                                                 GLenum textarget,
...
typedef void(APIENTRYP glDeleteFramebuffers_t)(GLsizei n,
...
struct GLManager::AttributeInfo {
  ...
  GLint location;
  AttributeInfo()
      : buf(0), p(0), floats_per_attribute(0), stride(0), offset(0),
        location(-1) {}
};
...
public:
  ...
  ShaderInfo(const string &name_, unsigned vshader_, unsigned fshader_,
      ...
      : name(name_), vshader(vshader_), fshader(fshader_), program(program_),
        have_vertex(false), have_normal(false), have_color3(false),
        have_color4(false), have_texcoord(false), checked(false) {
...
// Set up state
GLManager::GLManager()
    : current_shader(0), current_buffer(0), current_ibuffer(0),
      current_texture(0), flipping_textures(false), checked_restart(false),
      have_restart_fixed(false), have_restart(false), have_restart_nv(false),
...
unsigned GLManager::make_shader(const char *name, const char *vert_text,
  ...
  shaders.push_back(
      new ShaderInfo(name, vshader, fshader, program, nuniforms, nattributes));
  ...
  for (int i = 0; i < nuniforms; i++) {
    GLint size;
    GLenum type;
    glGetActiveUniform(program, i, len, NULL, &size, &type, &uname[0]);
  ...
  for (int i = 0; i < nattributes; i++) {
    GLint size;
    GLenum type;
    glGetActiveAttrib(program, i, len, NULL, &size, &type, &aname[0]);
...
bool GLManager::use_shader(unsigned ind) {
  ...
  for (size_t i = 0; i < current_shader->attribute_info.size(); i++) {
    ...
    if (ai.buf)
      attributearray(ai.location, ai.floats_per_attribute, ai.buf, ai.stride,
                     ai.offset);
    else if (ai.p)
      attributearray(ai.location, ai.floats_per_attribute, ai.p, ai.stride,
                     ai.offset);
  }
...
void GLManager::uniform1i(int ind, int x) {
  if (uniform_boilerplate(ind, GL_INT))
    glUniform1i(ind, x);
}
void GLManager::uniform2i(int ind, int x, int y) {
  if (uniform_boilerplate(ind, GL_INT_VEC2))
    glUniform2i(ind, x, y);
}
void GLManager::uniform3i(int ind, int x, int y, int z) {
  if (uniform_boilerplate(ind, GL_INT_VEC3))
    glUniform3i(ind, x, y, z);
}
void GLManager::uniform4i(int ind, int x, int y, int z, int w) {
  if (uniform_boilerplate(ind, GL_INT_VEC4))
    glUniform4i(ind, x, y, z, w);
}
void GLManager::uniform1f(int ind, float x) {
  if (uniform_boilerplate(ind, GL_FLOAT))
    glUniform1f(ind, x);
}
void GLManager::uniform2f(int ind, float x, float y) {
  if (uniform_boilerplate(ind, GL_FLOAT_VEC2))
    glUniform2f(ind, x, y);
}
void GLManager::uniform3f(int ind, float x, float y, float z) {
  if (uniform_boilerplate(ind, GL_FLOAT_VEC3))
    glUniform3f(ind, x, y, z);
}
void GLManager::uniform4f(int ind, float x, float y, float z, float w) {
  if (uniform_boilerplate(ind, GL_FLOAT_VEC4))
    glUniform4f(ind, x, y, z, w);
}
void GLManager::uniformMatrix2fv(int ind, const float *p) {
  if (uniform_boilerplate(ind, GL_FLOAT_MAT2))
    glUniformMatrix2fv(ind, 1, false, p);
}
void GLManager::uniformMatrix3fv(int ind, const float *p) {
  if (uniform_boilerplate(ind, GL_FLOAT_MAT3))
    glUniformMatrix3fv(ind, 1, false, p);
}
void GLManager::uniformMatrix4fv(int ind, const float *p) {
  if (uniform_boilerplate(ind, GL_FLOAT_MAT4))
    glUniformMatrix4fv(ind, 1, false, p);
}
...
void GLManager::attributearray(int ind, int floats_per_attribute,
  ...
  ai.stride = stride;
  ai.offset = offset;
  glVertexAttribPointer(ind, floats_per_attribute, GL_FLOAT, GL_FALSE, stride,
                        p + offset / sizeof(float));
...
void GLManager::attribute1f(int ind, float x) {
  if (attribute_boilerplate(ind, 1))
    glVertexAttrib1f(ind, x);
}
void GLManager::attribute2f(int ind, float x, float y) {
  if (attribute_boilerplate(ind, 2))
    glVertexAttrib2f(ind, x, y);
}
void GLManager::attribute3f(int ind, float x, float y, float z) {
  if (attribute_boilerplate(ind, 3))
    glVertexAttrib3f(ind, x, y, z);
}
void GLManager::attribute4f(int ind, float x, float y, float z, float w) {
  if (attribute_boilerplate(ind, 4))
    glVertexAttrib4f(ind, x, y, z, w);
}
...
void GLManager::update_ibuffer(unsigned buf, const unsigned *p, size_t ninds) {
  if (!use_ibuffer(buf))
    return;
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, ninds * sizeof(unsigned), p,
               GL_STATIC_DRAW);
...
void GLManager::flip_textures(bool flip) { flipping_textures = flip; }

// Textures
unsigned GLManager::make_texture(int w, int h, const void *p, unsigned format,
                                 unsigned type, unsigned internalformat) {
  if (!glGetProcs())
    return 0;
  unsigned texind;
...
void GLManager::update_texture(unsigned texind, int w, int h, const void *p,
  ...
  if (flipping_textures) {
    ...
    for (int i = 0; i < h; i++) {
      memcpy(buf + rowlen * (h - 1 - i), (const unsigned char *)p + rowlen * i,
             rowlen);
    }
...
bool GLManager::shader_ready() {
  ...
  for (size_t i = 0; i < current_shader->have_attribute.size(); i++) {
    ...
    if (strcmp(aname, current_shader->vertex_info.name.c_str()) == 0 &&
        current_shader->have_vertex)
      continue;
    if (strcmp(aname, current_shader->normal_info.name.c_str()) == 0 &&
        current_shader->have_normal)
      continue;
    if (strcmp(aname, current_shader->color_info.name.c_str()) == 0 &&
        (current_shader->have_color3 || current_shader->have_color4))
      continue;
    if (strcmp(aname, current_shader->texcoord_info.name.c_str()) == 0 &&
        current_shader->have_texcoord)
      continue;
...
void GLManager::draw_tris(const unsigned *p, std::size_t start,
                          std::size_t ntris) {
  shader_ready();
...
void GLManager::draw_tstrips(const unsigned *p, std::size_t start,
                             std::size_t nstripinds) {
  shader_ready();
  if (enable_restart()) {
    ...
    while (p < end) {
      ...
      while (*p != 0xffffffffu)
        p++;
      glDrawElements(GL_TRIANGLE_STRIP, p - stripstart, GL_UNSIGNED_INT,
                     stripstart);
      p++;
    }
  }
...
void GLManager::draw_tstrips(std::size_t start, std::size_t nstripinds) {
  ...
  if (enable_restart()) {
    ...
    if (!p) {
      eprintf("GLManager::draw_tstrips : drawing tstrips from IBO without "
...
void GLManager::check_clamp() {
  ...
  } else if (extension_supported("GL_ARB_color_buffer_float") && glGetProcs() &&
...
bool GLManager::shader_ok(unsigned shader_num, const char *what,
                          const char *name) {
  GLint status;
...
bool GLManager::program_ok(unsigned prog_num, const char *what,
                           const char *name) {
  GLint status;
