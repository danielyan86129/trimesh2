  ...
  for (int j = 0; j < tess_th; j++) {
    ...
    for (int i = 0; i < tess_ph; i++) {
      ...
      if (part == 0)
        ...
        mesh->vertices[i + j * tess_ph].set(
            2.0f + (2.0f - flipx * cos(u)) * cos(v), sin(u),
...
TriMesh *make_platonic(int nfaces) {
  ...
  default:
    TriMesh::eprintf(
        "make_platonic: Number of faces must be 4, 6, 8, 12, or 20\n");
    return NULL;
  }
}
...
// around the z axis.
TriMesh *make_surface_of_revolution(int tess_th,
                                    const vector<point> &curve_pts) {
