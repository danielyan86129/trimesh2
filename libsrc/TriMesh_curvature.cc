...
static void rot_coord_sys(const vec &old_u, const vec &old_v,
                          const vec &new_norm, vec &new_u, vec &new_v) {
  new_u = old_u;
  new_v = old_v;
...
// new_u, new_v basis.
void proj_curv(const vec &old_u, const vec &old_v, float old_ku, float old_kuv,
               float old_kv, const vec &new_u, const vec &new_v, float &new_ku,
               float &new_kuv, float &new_kv) {
  vec r_new_u, r_new_v;
...
void proj_dcurv(const vec &old_u, const vec &old_v, const Vec<4> old_dcurv,
  ...
  new_dcurv[0] =
      old_dcurv[0] * u1 * u1 * u1 + old_dcurv[1] * 3.0f * u1 * u1 * v1 +
  ...
  new_dcurv[3] =
      old_dcurv[0] * u2 * u2 * u2 + old_dcurv[1] * 3.0f * u2 * u2 * v2 +
      old_dcurv[2] * 3.0f * u2 * v2 * v2 + old_dcurv[3] * v2 * v2 * v2;
}

// Given a curvature tensor, find principal directions and curvatures
// Makes sure that pdir1 and pdir2 are perpendicular to normal
void diagonalize_curv(const vec &old_u, const vec &old_v, float ku, float kuv,
                      float kv, const vec &new_norm, vec &pdir1, vec &pdir2,
                      float &k1, float &k2) {
  vec r_old_u, r_old_v;
  ...
  if (likely(kuv != 0.0f)) {
    ...
    tt = (h < 0.0f) ? 1.0f / (h - sqrt(1.0f + h * h))
...
#pragma omp parallel for
  for (int i = 0; i < nf; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      ...
      // w[1][2] += u*v;
      vec dn =
          normals[faces[i][PREV_MOD3(j)]] - normals[faces[i][NEXT_MOD3(j)]];
      float dnu = dn DOT t;
      float dnv = dn DOT b;
      m[0] += dnu * u;
      m[1] += dnu * v + dnv * u;
...
#pragma omp parallel for
  for (int i = 0; i < nv; i++) {
    diagonalize_curv(pdir1[i], pdir2[i], curv1[i], curv12[i], curv2[i],
                     normals[i], pdir1[i], pdir2[i], curv1[i], curv2[i]);
  }
  dprintf("Done.\n");
}
...
#pragma omp parallel for
  for (int i = 0; i < nf; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      int vj = faces[i][j];
      proj_curv(pdir1[vj], pdir2[vj], curv1[vj], 0, curv2[vj], t, b,
                fcurv[j][0], fcurv[j][1], fcurv[j][2]);
    }
