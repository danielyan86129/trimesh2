...
crossed:
  ...
  size_t split_left = (num_leq <= ideal_split)
                          ? num_leq
                          : (num_less >= ideal_split) ? num_less : ideal_split;
...
void KDtree::Node::find_closest_compat_to_pt(
    KDtree::Node::Traversal_Info &ti) const {
  if (npts) {
    for (int i = 0; i < npts; i++) {
      float myd2 = dist2(leaf.p[i], ti.p);
      if (myd2 < ti.closest_d2 && leaf.p[i] != ti.p &&
...
void KDtree::Node::find_closest_compat_to_ray(
    KDtree::Node::Traversal_Info &ti) const {
  if (npts) {
    for (int i = 0; i < npts; i++) {
      float myd2 = dist2ray2(leaf.p[i], ti.p, ti.dir);
...
void KDtree::Node::find_k_closest_to_pt(
    KDtree::Node::Traversal_Info &ti) const {
  // Leaf nodes
  if (npts) {
    for (int i = 0; i < npts; i++) {
...
// Same as above, with compat
void KDtree::Node::find_k_closest_compat_to_pt(
    KDtree::Node::Traversal_Info &ti) const {
  if (npts) {
    for (int i = 0; i < npts; i++) {
      float myd2 = dist2(leaf.p[i], ti.p);
...
void KDtree::find_k_closest_to_pt(std::vector<const float *> &knn, int k,
                                  const float *p, float maxdist2 /* = 0.0f */,
                                  const CompatFunc *iscompat /* = NULL */,
                                  float approx_eps /* = 0.0f */) const {
  knn.clear();
  if (!root || !p)
