...
static inline float wt(const TriMesh *themesh, int v1, int v2,
                       float invsigma2) {
...
template <class T> struct AccumVec {
  const vector<T> &field;
  AccumVec(const vector<T> &field_) : field(field_) {}
  inline void operator()(const TriMesh *, int /* v0 */, T &f, float w,
                         int v) const {
    f += w * field[v];
  }
};
...
struct AccumCurv {
  inline void operator()(const TriMesh *themesh, int v0, vec &c, float w,
                         int v) const {
    vec ncurv;
    proj_curv(themesh->pdir1[v], themesh->pdir2[v], themesh->curv1[v], 0,
              themesh->curv2[v], themesh->pdir1[v0], themesh->pdir2[v0],
...
struct AccumDCurv {
  inline void operator()(const TriMesh *themesh, int v0, Vec<4> &d, float w,
                         int v) const {
    Vec<4> ndcurv;
    proj_dcurv(themesh->pdir1[v], themesh->pdir2[v], themesh->dcurv[v],
               themesh->pdir1[v0], themesh->pdir2[v0], ndcurv);
    d += w * ndcurv;
  }
};
...
static void diffuse_vert_field(TriMesh *themesh, vector<unsigned> &flags,
                               unsigned &flag_curr, const ACCUM &accum, int v,
                               float invsigma2, T &flt) {
...
#pragma omp for
    for (int i = 0; i < nv; i++) {
      diffuse_vert_field(themesh, flags, flag_curr,
                         AccumVec<vec>(themesh->vertices), i, invsigma2,
                         dflt[i]);
...
#pragma omp for
    for (int i = 0; i < nv; i++) {
      diffuse_vert_field(themesh, flags, flag_curr, AccumVec<point>(dflt), i,
                         invsigma2, dflt2[i]);
    }
...
static void jones_filter(TriMesh *themesh, vector<unsigned> &flags,
                         unsigned &flag_curr, int v, float invsigma2_1,
...
#pragma omp for
    for (int i = 0; i < nv; i++)
      jones_filter(themesh, flags, flag_curr, i, invsigma2_1, invsigma2_2,
                   oldverts);
  }
...
#pragma omp for
    for (int i = 0; i < nv; i++)
      diffuse_vert_field(themesh, flags, flag_curr, a, i, invsigma2, flt[i]);
  } // #pragma omp parallel
...
#pragma omp for
    for (int i = 0; i < nv; i++)
      diffuse_vert_field(themesh, flags, flag_curr, AccumCurv(), i, invsigma2,
                         cflt[i]);

#pragma omp for
    for (int i = 0; i < nv; i++)
      diagonalize_curv(themesh->pdir1[i], themesh->pdir2[i], cflt[i][0],
                       cflt[i][1], cflt[i][2], themesh->normals[i],
                       themesh->pdir1[i], themesh->pdir2[i], themesh->curv1[i],
                       themesh->curv2[i]);
  } // #pragma omp parallel
...
#pragma omp for
    for (int i = 0; i < nv; i++)
      diffuse_vert_field(themesh, flags, flag_curr, AccumDCurv(), i, invsigma2,
                         dflt[i]);
  } // #pragma omp parallel
...
template void diffuse_vector<Vec<2, float>>(TriMesh *, vector<Vec<2, float>> &,
                                            float);
template void diffuse_vector<Vec<3, float>>(TriMesh *, vector<Vec<3, float>> &,
                                            float);
template void diffuse_vector<Vec<4, float>>(TriMesh *, vector<Vec<4, float>> &,
                                            float);
