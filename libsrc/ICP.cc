...
void compute_overlaps(TriMesh *mesh1, TriMesh *mesh2, const xform &xf1,
                      const xform &xf2, const KDtree *kd1, const KDtree *kd2,
                      vector<float> &o1, vector<float> &o2, float maxdist,
                      int verbose) {
  timestamp t = now();
...
static void select_and_match(TriMesh *mesh1, TriMesh *mesh2, const xform &xf1,
                             const xform &xf2, const KDtree *kd2,
  ...
  bool is_pointcloud1 =
      (mesh1->faces.empty() && mesh1->tstrips.empty() && mesh1->grid.empty());
  bool is_pointcloud2 =
      (mesh2->faces.empty() && mesh2->tstrips.empty() && mesh2->grid.empty());
...
static void align_symm(const vector<PtPair> &pairs, float scale,
  ...
  alignxf = xform::trans(centroid1) * R * xform::trans(trans) * R *
...
static void align_scale(const vector<PtPair> &pairs, xform &alignxf,
  ...
  alignxf = xform::trans(centroid) * xform::fromarray(csqrt1) *
            xform::fromarray(icsqrt2) * xform::trans(-centroid) * alignxf;
}
...
static float ICP_iter(TriMesh *mesh1, TriMesh *mesh2, const xform &xf1,
                      xform &xf2, const KDtree *kd1, const KDtree *kd2,
                      const vector<float> &weights1,
                      const vector<float> &weights2, vector<float> &sampcdf1,
                      vector<float> &sampcdf2, int desired_pairs,
                      float &cdfincr, bool update_cdfs, float &maxdist,
                      float &angle_thresh, int verbose,
                      ICP_xform_type xform_type) {
  const int nv1 = mesh1->vertices.size(), nv2 = mesh2->vertices.size();
  ...
  select_and_match(mesh1, mesh2, xf1, xf2, kd2, sampcdf1, cdfincr, maxdist,
                   angle_thresh, false, pairs);
  select_and_match(mesh2, mesh1, xf2, xf1, kd1, sampcdf2, cdfincr, maxdist,
                   angle_thresh, true, pairs);
  ...
  angle_thresh = clamp(ANGLE_THRESH_MULT * median_angle, ANGLE_THRESH_MIN,
                       ANGLE_THRESH_MAX);
  float normdot_thresh = cos(angle_thresh);
  ...
  for (int i = 0; i < nv2; i++) {
    ...
    for (int j = 0; j < 3; j++)
      for (int k = j; k < 3; k++)
        A2[j][k] += mesh2->normals[i][j] * mesh2->normals[i][k];
  }
  ...
  for (int i = 0; i < nv1; i++) {
    ...
    float s = 0.0f;
    for (int j = 0; j < 3; j++)
      s += eval1[j] * sqr(A1[0][j] * mesh1->normals[i][0] +
  ...
  for (int i = 0; i < nv2; i++) {
    ...
    float s = 0.0f;
    for (int j = 0; j < 3; j++)
      s += eval2[j] * sqr(A2[0][j] * mesh2->normals[i][0] +
...
static void make_uniform_cdfs(const vector<float> &weights1,
                              vector<float> &sampcdf1,
                              const vector<float> &weights2,
...
float ICP(TriMesh *mesh1, TriMesh *mesh2, const xform &xf1, xform &xf2,
          const KDtree *kd1, const KDtree *kd2, vector<float> &weights1,
          vector<float> &weights2, float maxdist /* = 0.0f */,
  ...
  for (iter = 0; iter < MAX_ITERS; iter++) {
    ...
    if (recompute) {
      if (!had_weights) {
        compute_overlaps(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2,
                         maxdist, verbose);
      }
    ...
    // Do an iteration
    err = ICP_iter(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2,
                   sampcdf1, sampcdf2, DESIRED_PAIRS, cdfincr, recompute,
                   maxdist, angle_thresh, verbose, iter_xform_type);
    ...
    if (iter - iter_of_min_err >= TERMINATION_ITER_THRESH &&
        iter_of_min_err >= 0 && !recompute && xform_type != ICP_SIMILARITY &&
  ...
  for (iter = 0; iter < FINAL_ITERS; iter++) {
    ...
    err = ICP_iter(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2,
                   sampcdf1, sampcdf2, DESIRED_PAIRS_FINAL, cdfincr, false,
                   maxdist, angle_thresh, verbose, iter_xform_type);
    if (err < 0) {
      if (!had_weights) {
        weights1.clear();
...
float ICP(TriMesh *mesh1, TriMesh *mesh2, const xform &xf1, xform &xf2,
          const KDtree *kd1, const KDtree *kd2, int verbose /* = 0 */,
          ICP_xform_type xform_type /* = ICP_RIGID */) {
  vector<float> weights1, weights2;
  return ICP(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2, 0.0f,
             verbose, xform_type);
}
...
#pragma omp parallel for
  ...
  vector<float> weights1, weights2;
  float icperr = ICP(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2, 0.0f,
                     verbose, xform_type);
  delete kd2;
  delete kd1;
  return icperr;
...
float ICP(TriMesh *mesh1, TriMesh *mesh2, const xform &xf1, xform &xf2,
          const KDtree *kd1, const KDtree *kd2, ::std::vector<float> &weights1,
          ::std::vector<float> &weights2, float maxdist, int verbose,
          bool do_scale, bool do_affine) {
  ICP_xform_type xform_type =
      do_affine ? ICP_AFFINE : do_scale ? ICP_SIMILARITY : ICP_RIGID;
  return ICP(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2, maxdist,
             verbose, xform_type);
}

float ICP(TriMesh *mesh1, TriMesh *mesh2, const xform &xf1, xform &xf2,
          const KDtree *kd1, const KDtree *kd2, int verbose, bool do_scale,
          bool do_affine) {
  ICP_xform_type xform_type =
      do_affine ? ICP_AFFINE : do_scale ? ICP_SIMILARITY : ICP_RIGID;
