...
template <class Array> class CompareArrayElements {
private:
  const Array &a;
...
static bool connected(const TriMesh *mesh, int f1, int f2, bool conn_vert) {
  int f10 = mesh->faces[f1][0], f11 = mesh->faces[f1][1],
      f12 = mesh->faces[f1][2];
  int f20 = mesh->faces[f2][0], f21 = mesh->faces[f2][1],
      f22 = mesh->faces[f2][2];

  if (conn_vert)
    return f10 == f20 || f10 == f21 || f10 == f22 || f11 == f20 || f11 == f21 ||
           f11 == f22 || f12 == f20 || f12 == f21 || f12 == f22;
  else
    return (f10 == f20 && (f11 == f22 || f12 == f21)) ||
           (f10 == f21 && (f11 == f20 || f12 == f22)) ||
           (f10 == f22 && (f11 == f21 || f12 == f20)) ||
           (f11 == f20 && f12 == f22) || (f11 == f21 && f12 == f20) ||
           (f11 == f22 && f12 == f21);
}

// Helper function for find_comps, below.  Finds and marks all the faces
// connected to f.
static void find_connected(const TriMesh *mesh, vector<int> &comps,
                           vector<int> &compsizes, int f, int whichcomponent,
                           bool conn_vert) {
  stack<int> s;
  s.push(f);
  while (!s.empty()) {
...
void select_big_comps(
    TriMesh *mesh, const vector<int> &comps, const vector<int> &compsizes,
