...
static void find_overlap_onedir(TriMesh *mesh1, TriMesh *mesh2,
                                ...
                                const KDtree *kd2, float &overlap_area,
                                float &nonoverlap_area, float &rmsdist) {
  overlap_area = 0.0f;
  rmsdist = 0.0f;
  ...
  for (int i = 0; i < nsamp; i++) {
    ...
    if (mesh2->is_bdy(ind2))
      continue;
    if (((xf12r * mesh1->normals[ind]) DOT mesh2->normals[ind2]) <= 0.0f)
      continue;
    overlap_area += this_area;
    rmsdist += this_area * sqr((p - point(q)) DOT mesh2->normals[ind2]);
  }
...
void find_overlap(TriMesh *mesh1, TriMesh *mesh2, const xform &xf1,
